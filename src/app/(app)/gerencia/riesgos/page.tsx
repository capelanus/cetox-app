...
export default async function RiesgosPage() {
  ...
  const [riesgos, plan, usuarios] = await Promise.all([
    prisma.riesgo.findMany({ orderBy: { createdAt: 'desc' } }),
  ...
  const riesgosSer = riesgos.map(r => ({ ...r, fechaRevision: r.fechaRevision?.toISOString() ?? null }))
  return <RiesgosClient riesgos={riesgosSer} objetivos={objetivos} usuarios={usuarios} />
}
