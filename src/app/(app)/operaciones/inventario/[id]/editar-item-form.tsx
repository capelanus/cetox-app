...
export default function EditarItemForm({ item }: { item: ItemData }) {
  ...
  const eliminar   = eliminarInventarioItem.bind(null, item.id)
        ...
        <form action={handleSubmit} className="px-5 pb-5 space-y-4 border-t border-gray-100 pt-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={e => {
                const input = e.target.form?.querySelector<HTMLInputElement>('[name="activo"][type="hidden"]')
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <form action={eliminar} className="ml-auto">
              <button
                type="submit"
                onClick={e => { if (!confirm('¿Eliminar este ítem del inventario? Esta acción no se puede deshacer.')) e.preventDefault() }}
